  ...
  const handleBlurLocal = (e) => {
    if (isPrice) {
      ...
      if (isNaN(numValue) || numValue === 0) {
        ...
        onBlur({ target: { name, value: formatNumber(defaultValue) } });
      } else {
        onBlur({ target: { name, value: formatNumber(numValue) } });
      }
    } else {
      onBlur(e);
    }
...
const EquipoRow = ({ titulo, nombreCantidad, nombrePrecio, form, onChange, onBlur, unidadLabel, children }) => {
  ...
  const handlePrecioChange = (e) => {
    ...
    const numValue = parseFloat(clean) || 0;
    const formatted = formatNumber(numValue);
    onChange({ target: { name: nombrePrecio, value: formatted } });
  };
  ...
  const handlePrecioBlur = (e) => {
    const numValue = parseFormattedNumber(precioRaw);
    if (isNaN(numValue) || numValue === 0) {
    ...
    <div className="equipo-row">
      ...
      <div className="equipo-grid">
        ...
        <div className="input-group total-field">
          ...
          <span className="hint">{cant} × {new Intl.NumberFormat("es-CO").format(precio)}</span>
        </div>
        {children}
      </div>
    </div>
...
export default function Formulario({ onSimular }) {
  ...
  const handleBlur = (e) => {
    const { name } = e.target;
    if (name === "clima" || name === "tipo_cable" || name === "caida_voltaje") return;
  ...
  const totalPaneles = (parseFloat(form.cantidad_paneles) || 0) * (parseFormattedNumber(form.precio_panel) || 0);
  const totalBaterias = (parseFloat(form.cantidad_baterias) || 0) * (parseFormattedNumber(form.precio_bateria) || 0);
  const totalMppt = (parseFloat(form.cantidad_mppt) || 0) * (parseFormattedNumber(form.precio_mppt) || 0);
  const totalAires = (parseFloat(form.cantidad_aires) || 0) * (parseFormattedNumber(form.precio_aire) || 0);
  const granTotal = totalPaneles + totalBaterias + totalMppt + totalAires;

  const cop = (n) =>
    new Intl.NumberFormat("es-CO", { style: "currency", currency: "COP", maximumFractionDigits: 0 }).format(n);

  const kwTotales = (parseFloat(form.cantidad_paneles) || 0) * (parseFloat(form.panel_kw) || 0);
  const kwhTotales = (parseFloat(form.cantidad_baterias) || 0) * (parseFloat(form.bateria_kwh) || 0);
  const btuTotales = (parseFloat(form.cantidad_aires) || 0) * (parseFloat(form.btu_aire) || 0);
  ...
  return (
    <form onSubmit={handleSubmit} className="formulario-solar">
      ...
      <section className="form-section">
        <h3 className="section-title"><span className="section-icon">☀️</span> Paneles Solares</h3>
        <EquipoRow
          titulo="Panel solar"
          ...
          unidadLabel="paneles"
        >
          <InputField label="Potencia por panel" name="panel_kw" value={form.panel_kw} onChange={handleChange} onBlur={handleBlur} unit="kW" hint={`Total instalado: ${kwTotales.toFixed(2)} kW`} />
        </EquipoRow>
        <div className="inputs-grid" style={{ marginTop: "0.75rem" }}>
          <InputField label="Horas de sol pico" name="horas_sol" value={form.horas_sol} onChange={handleChange} onBlur={handleBlur} unit="HSP/día" hint="Neiva aprox. 4.5-5.5 HSP" />
          <InputField label="Eficiencia sistema" name="eficiencia" value={form.eficiencia} onChange={handleChange} onBlur={handleBlur} unit="%" hint="Perdidas cables e inversor" />
          <InputField label="Temperatura ambiente" name="temperatura" value={form.temperatura} onChange={handleChange} onBlur={handleBlur} unit="°C" hint="Afecta degradacion termica" />
          <InputField label="Anos de proyeccion" name="anos" value={form.anos} onChange={handleChange} onBlur={handleBlur} unit="años" hint="Degradacion acumulada" />
        </div>
      </section>
      ...
      <section className="form-section">
        ...
        >
          <InputField label="Capacidad por bateria" name="bateria_kwh" value={form.bateria_kwh} onChange={handleChange} onBlur={handleBlur} unit="kWh" hint={`Total banco: ${kwhTotales.toFixed(1)} kWh`} />
        </EquipoRow>
        <div className="inputs-grid" style={{ marginTop: "0.75rem" }}>
          <InputField label="Ciclos de vida" name="ciclos_bateria" value={form.ciclos_bateria} onChange={handleChange} onBlur={handleBlur} unit="ciclos" hint="Litio aprox. 3000-6000 ciclos" />
        </div>
      </section>

      {/* ── MPPT ── */}
      <section className="form-section">
        <h3 className="section-title"><span className="section-icon">⚡</span> Controladores MPPT</h3>
        <EquipoRow
      ...
      <section className="form-section">
        <h3 className="section-title"><span className="section-icon">❄️</span> Aires Acondicionados DC 48V</h3>
        <EquipoRow
          ...
          unidadLabel="aires"
        >
          <InputField label="BTU por unidad" name="btu_aire" value={form.btu_aire} onChange={handleChange} onBlur={handleBlur} unit="BTU" hint={`Total BTU instalados: ${new Intl.NumberFormat("es-CO").format(btuTotales)}`} />
        </EquipoRow>
      </section>

      {/* ── RESUMEN DE INVERSION ── */}
      <section className="form-section inversion-resumen">
        <h3 className="section-title"><span className="section-icon">💰</span> Resumen de Inversion</h3>
      ...
      <section className="form-section">
        ...
        <div className="inputs-grid">
          <InputField label="Area a climatizar" name="area" value={form.area} onChange={handleChange} onBlur={handleBlur} unit="m²" hint="Para calcular BTU necesarios" />
          <InputField label="Consumo diario" name="consumo_diario" value={form.consumo_diario} onChange={handleChange} onBlur={handleBlur} unit="kWh/día" hint="Ver recibo Electrohuila" />
          <InputField label="Precio del kWh" name="precio_kwh" value={form.precio_kwh} onChange={handleChange} onBlur={handleBlur} unit="COP/kWh" hint="Tarifa Electrohuila aprox. 900" />
          <InputField label="Mantenimiento mensual" name="mantenimiento_mensual" value={form.mantenimiento_mensual} onChange={handleChange} onBlur={handleBlur} unit="COP/mes" hint="Limpieza, revision, ajustes" />
        </div>
      </section>

      {/* ── VOLTAJE DEL SISTEMA ── */}
      <section className="form-section">
        <h3 className="section-title"><span className="section-icon">⚡</span> Voltaje del Sistema</h3>
        <div className="inputs-grid">
          <div className="input-group span-full">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setForm((prev) => ({
                    ...
                    voltaje_custom: newValue === "custom" ? prev.voltaje_custom : "",
                  }));
                }}
              >
                <option value="12">🔋 12V (sistemas pequeños)</option>
                <option value="24">🔋 24V (sistemas medianos)</option>
                <option value="48">🔋 48V (sistemas grandes - recomendado)</option>
            ...
            <span className="hint">Define el voltaje nominal de tu banco de baterías. Afecta el cálculo de amperaje.</span>
          </div>
        </div>
      </section>
      ...
      <section className="form-section">
        <h3 className="section-title"><span className="section-icon">🔌</span> Cableado DC (Opcional)</h3>
        <div className="inputs-grid">
          <InputField label="Distancia paneles a baterías" name="distancia_paneles" value={form.distancia_paneles} onChange={handleChange} onBlur={handleBlur} unit="metros" hint="Distancia desde paneles hasta MPPT/baterías" />
          <div className="input-group">
            <label htmlFor="tipo_cable">Tipo de cable</label>
            <select id="tipo_cable" name="tipo_cable" value={form.tipo_cable || "cobre"} onChange={handleChange}>
              <option value="cobre">🔴 Cobre (recomendado)</option>
              <option value="aluminio">⚪ Aluminio (económico)</option>
            </select>
            <span className="hint">Cobre mejor conductividad, aluminio más económico</span>
          </div>
          <div className="input-group">
            <label htmlFor="caida_voltaje">Caída de voltaje máxima</label>
            <select id="caida_voltaje" name="caida_voltaje" value={form.caida_voltaje || "2"} onChange={handleChange}>
            ...
            <span className="hint">Menor % = mejor eficiencia pero cable más caro</span>
          </div>
        </div>
      </section>
